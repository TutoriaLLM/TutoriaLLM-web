...
<Aside type="note" title="開発を行わない場合">
  開発を行わない場合は、[Docker
  Hub](https://hub.docker.com/r/soumame/code-tutorial-app)からイメージを利用できます。PostgreSQL,
  Redis と接続する必要があります。
</Aside>
...
<Aside type="note" title="ノート">
  ...
  の`docker-compose.yml`ファイルを参照しています。このファイルは、開発向けのファイルのため、[実稼働アプリをビルドする](/editorguides/deployment)
  場合は`docker-compose.prod.yml`ファイルを利用することをお勧めします。
</Aside>

import composeExample from "./code/docker-compose.yml?raw";
...
import envExample from "./code/.env.dev.example?raw";
